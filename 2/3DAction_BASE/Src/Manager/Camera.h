...
public:
	...
	static constexpr float LIMIT_X_UP_DEG = 40.0f;
	static constexpr float LIMIT_X_DW_DEG = 15.0f ;
	static constexpr float LIMIT_X_UP_RAD = LIMIT_X_UP_DEG * (DX_PI_F / 180.0f);
	static constexpr float LIMIT_X_DW_RAD = LIMIT_X_DW_DEG * (DX_PI_F / 180.0f);
	
	// 追従対象との調整座標
	static constexpr VECTOR RELATIVE_TRANSFORM_POS = { 0.0f, 20.0f, 0.0f };

	static constexpr float CAMERA_ANGLE_SPEED_Y = 3.0f;
	static constexpr float CAMERA_ANGLE_SPEED_X = 1.0f;
